...
describe(opcode(0x33, 2, 5, 1, function TempBasal (vars) {
  vars.rate = vars.head[1] / 40.0;
  return vars;
}));

describe(opcode(0x16, 2, 5, 0, function TempBasalDuration (vars) {
  vars.minutes = vars.head[1] * 30;
  return vars;
}));
...
function eat_nulls (data) {
  ...
  // TODO: There's a bug here where the last records on a page actually have
  // valid nulls at the end of the record.  Bug likely to express when Basal
  // changes or descriptions which likely end in nulls are at the end of a
  // page.
  return data;
  return Buffer.concat([data, new Buffer('000000', 'hex')]);
}
